...
def proposal_layer(rpn_cls_prob_list, rpn_bbox_pred_list, im_info, cfg_key, _feat_stride, anchors, num_anchors_list):
  """
    rpn_cls_prob = [n, h, w, c=a*2]
    rpn_bbox_pred = [n, h, w, c=4*a]
  """
  assert len(rpn_bbox_pred_list) == len(rpn_cls_prob_list)
  ...
  scores_list = list()
  for index in range(len(rpn_cls_prob_list)):
    rpn_cls_prob = rpn_cls_prob_list[index]
    num_anchors = num_anchors_list[index]
  ...
  rpn_bbox_pred = [bbox_pred.view((-1,4)) for bbox_pred in rpn_bbox_pred_list]
  rpn_bbox_pred = torch.cat(rpn_bbox_pred, dim=0)
