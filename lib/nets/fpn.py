...
class Resnet_Ori(nn.Module):
  ...
  def _make_layer(self, block, planes, blocks, stride=1, dilated=False):
    ...
    layers = []
    layers.append(block(self.inplanes, planes, stride, downsample, dilated))
...
class FPN_Resnet(Network):
  def __init__(self, resnet_type, feat_strdie=(4,8,16,32,64), anchor_scales=(32,64,128,256,512), anchor_ratios=(0.5, 1, 2)):
  ...
  def _init_bottom_up(self):
    ...
    self._layers['c2'] = nn.Sequential(self._resnet.conv1,
                                         self._resnet.bn1,
                                         self._resnet.relu,
                                       self._resnet.maxpool,
                                       self._resnet.layer1)
    self._layers['c3'] = self._resnet.layer2
    self._layers['c4'] = self._resnet.layer3
    self._layers['c5'] = self._resnet.layer4

  def _init_top_down(self):
    # init_lateral_connection
    self.c2_lateral_conv = nn.Conv2d(64 * self._resnet.block.expansion, self._lateral_channel,
                                     kernel_size=1, bias=True, stride=1)
    self.c3_lateral_conv = nn.Conv2d(128 * self._resnet.block.expansion, self._lateral_channel,
                                     kernel_size=1, bias=True, stride=1)
    self.c4_lateral_conv = nn.Conv2d(256 * self._resnet.block.expansion, self._lateral_channel,
                                     kernel_size=1, bias=True, stride=1)
    self.c5_lateral_conv = nn.Conv2d(512 * self._resnet.block.expansion, self._lateral_channel,
                                     kernel_size=1, bias=True, stride=1)

    self.p6_conv = nn.MaxPool2d(kernel_size=2, stride=2)
    self._layers['p5'] = self.c5_lateral_conv
    self._layers['p6'] = self.p6_conv

    # self.p5_p4_upsample = nn.Upsample(scale_factor=2, mode='bilinear')
    # self._layers['p5_p4'] = self.p5_p4_upsample
    self._layers['p5_p4_lateral'] = self.c4_lateral_conv
    self.p4_conv = nn.Conv2d(self._lateral_channel, self._lateral_channel,
                        kernel_size=3, bias=True, stride=1, padding=1)
    self._layers['p4'] = self.p4_conv

    # self.p4_p3_upsample = nn.Upsample(scale_factor=2, mode='bilinear')
    # self._layers['p4_p3'] = self.p4_p3_upsample
    self._layers['p4_p3_lateral'] = self.c3_lateral_conv
    self.p3_conv = nn.Conv2d(self._lateral_channel, self._lateral_channel,
                        kernel_size=3, bias=True, stride=1, padding=1)
    self._layers['p3'] = self.p3_conv

    # self.p3_p2_upsample = nn.Upsample(scale_factor=2, mode='bilinear')
    # self._layers['p3_p2'] = self.p3_p2_upsample
    self._layers['p3_p2_lateral'] = self.c2_lateral_conv
    self.p2_conv = nn.Conv2d(self._lateral_channel, self._lateral_channel,
                        kernel_size=3, bias=True, stride=1, padding=1)
    self._layers['p2'] = self.p2_conv


  def _init_network(self):
    self._init_bottom_up()
