...
class BIG_ROI_Pooling(nn.Module):
  def __init__(self, feat_strides,pooled_height=15, pooled_width=15):
    super(BIG_ROI_Pooling, self).__init__()
  ...
  def enlarge_rois(self,rois):
    t = Variable(torch.Tensor(rois.shape)).cuda()
    wp = (rois[:, 3] - rois[:, 1])*1
    hp = (rois[:, 4] - rois[:, 2])*0.5
    t[:, 0] = rois[:, 0]
    t[:, 1] = rois[:, 1]-wp
    t[:, 2] = rois[:, 2]-hp
    t[:, 3] = rois[:, 3]+wp
  ...
  def forward(self, features, rois):
    ...
    if rois[0] is not None:
      t = self.enlarge_rois(rois[0])
      feat_p3 = self.roi_pool_p3(features[0], t)
      feat_list.append(feat_p3)
    if rois[1] is not None:
      t = self.enlarge_rois(rois[1])
      feat_p4 = self.roi_pool_p4(features[1], t)
      feat_list.append(feat_p4)
    if rois[2] is not None:
      t = self.enlarge_rois(rois[2])
      feat_p5 = self.roi_pool_p5(features[2], t)
      feat_list.append(feat_p5)
    if rois[3] is not None:
      t = self.enlarge_rois(rois[3])
      feat_p6 = self.roi_pool_p6(features[3], t)
      feat_list.append(feat_p6)
