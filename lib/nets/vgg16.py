...
class VGG16(Network):
  def __init__(self, feat_strdie=(16, ), anchor_scales=(8, 16, 32), anchor_ratios=(0.5, 1, 2)):
    ...
    # print('ratios:-------', self._anchor_scales)
  ...
  def _load_pre_trained_model(self, pre_trained_model):
    ...
    state_dict['features.24.weight'] = state_dict.pop('features.23.weight')
    state_dict['features.24.bias']   = state_dict.pop('features.23.bias')
    state_dict['features.26.weight'] = state_dict.pop('features.25.weight')
    state_dict['features.26.bias']   = state_dict.pop('features.25.bias')
    state_dict['features.28.weight'] = state_dict.pop('features.27.weight')
    state_dict['features.28.bias']   = state_dict.pop('features.27.bias')
    # state_dict = self._vgg.state_dict()
    ...
    pre_model_dict['features.23.weight'] = pre_model_dict.pop('features.24.weight')
    pre_model_dict['features.23.bias']   = pre_model_dict.pop('features.24.bias')
    pre_model_dict['features.25.weight'] = pre_model_dict.pop('features.26.weight')
    pre_model_dict['features.25.bias']   = pre_model_dict.pop('features.26.bias')
    pre_model_dict['features.27.weight'] = pre_model_dict.pop('features.28.weight')
    pre_model_dict['features.27.bias']   = pre_model_dict.pop('features.28.bias')
